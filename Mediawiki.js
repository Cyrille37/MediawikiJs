 ...
 *
 * @author Cyrille Giquello at comptoir.net -> since https://github.com/Wikia/app/commit/1cef2eb84c68b5db4af9fe7449606cae951317f6#diff-ea366348f95d0f24b5b97da23126a7a8
...
Mediawiki.apiCall = function(apiParams, callbackSuccess, callbackError, method, ajaxParams) {
   ...
   try
   {
	...
	var p = {
		...
		data: apiParams,
		type : method || 'GET',
		dataType: apiParams.format, // Tell jQuery that the result is json so it returns a javascript object
		xhrFields: {	// important! for CORS working
...
Mediawiki.editArticle = function (article, callbackSuccess, callbackError)
{
	try
	{
		Mediawiki.getToken(article.title, 'edit',
			/**
			 * Success
			 */
			function(editToken)
			{
				console.log('Mediawiki.editArticle getToken Success');
				...
				return Mediawiki.apiCall(apiParams,
					/**
					 * Success
					 */
					function(data)
					{
						var cresult = Mediawiki.checkResult(data);
						if (cresult !== true) {
			...
			function(data)
			{
				console.log('Mediawiki.editArticle getToken Error');
				console.log(data);
			}
		);
...
Mediawiki.search = function( what, search, callbackSuccess, callbackError )
{
	try {
		...
		return Mediawiki.apiCall(p, callbackSuccess, callbackError);
		
	} catch (e) {
		Mediawiki.error("Error searching article");
		Mediawiki.d(Mediawiki.print_r(e));
		return false;
	}
 ...
 * @param title (could support multiple if someone wants to build it out)
 * @tokenType - type of token. Example "edit"
 */
Mediawiki.getToken = function(titles, tokenType, callbackSuccess, callbackError)
{
	...
	var responseData = Mediawiki.apiCall(
		...
		function( data )
		{
			...
			for( var pi in data.query.pages )
			{
				var page = data.query.pages[pi];
				if( page.title == titles )
				{
					callbackSuccess(page.edittoken);
					return ;
				}
			}
		...
		{
			//console.log(data);
			Mediawiki.error("API Error obtaining token");
			callbackError(data);
		}
	);
	...
	/*
	// We can get two different responses back here.
	// If it's a valid title, then it returns it
...
Mediawiki.login = function (username, password, callbackSuccess, callbackError, token, cookiePrefix, sessionId){
	...
	if (Mediawiki.isLoggedIn()){
		Mediawiki.d('You are already logged in as "'+Mediawiki.UserName+'"');
		return null;
	}
...
Mediawiki.updateStatus = function(msg, isError, timeout){
	...
	if ( isError ){
		...
		Mediawiki.statusBar.show(msg, timeout || Mediawiki.statusBarTimeout, true);
	} else {
		Mediawiki.statusBar.show(msg, timeout || Mediawiki.statusBarTimeout, false);
	}
};
...
var MediawikiStatusBar = function (sel,options) {
	...
	if (!_sb) {
		_sb = $('<div id="_statusbar" class="' + _I.cssClass + '">' + '</div>' ).appendTo(document.body).show();
		$('.' + _I.cssClass).click(function(e) {
			//_I.timeout = null ;
			//$(_sb).fadeOut();
			_I.release();
		});
	...
	this.show = function(message,timeout,isError) {
		...
		if( _I.timeout ) {
			clearTimeout(_I.timeout);
			_I.timeout = null ;
		}

		if (_I.additive) {
			var html = '<div style="margin-bottom: 2px;" >Mediawiki: ' + message + "</div>";
		...
		if( ! isError)
		{
			_I.timeout = window.setTimeout( function(){ _I.release(); }, timeout || _I.closeTimeout);
		}
